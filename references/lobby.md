...
Full-width strip pinned to the top of the content area, height ≈ 8% of viewport height. Same flat dark strip language as the mode-select top bar's structural role, but with DIFFERENT left content — this is effectively a sub-header, not the identity bar.
...
- **Shape**: rounded-rect, corner radius ~8px, aspect ratio roughly 2.2:1 (wide, short) — same shape for every empty slot regardless of column.
- **Size**: each empty slot height ≈ 14-16% of the main content region's height; width ≈ 30% of content width (so 3 columns + gutters fit).
...
- **Anchor**: center column, vertically centered in the main content region (or centered within its column's 2-cell stack per our 8-slot build).
- **Shape**: rounded-rect card, larger than the empty slots — width ≈ same as an empty slot's width (~30% of content width) but HEIGHT ≈ 1.4-1.6× an empty slot's height (to fit the richer internal content), corner radius ~8px, with a visible border (distinct from the empty slots' faint outline — this one reads as a populated, designed card).
